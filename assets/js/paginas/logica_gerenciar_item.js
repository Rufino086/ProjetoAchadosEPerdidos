document.addEventListener("DOMContentLoaded", () => {
    ...
    async function carregarItem(id) {
        try {
            ...
            const podeEditar = usuarioVerificado && (
                usuarioVerificado.id === item.id_usuario_reportou || 
                (usuarioVerificado.tipo === "ponto_coleta" && usuarioVerificado.id === item.id_ponto_coleta)
            );
            ...
            if (podeEditar) {
                ...
                if (usuarioVerificado) {
                    ...
                    if (item.tipo_registro === "perdido" && item.status === "Perdido" && usuarioVerificado.id === item.id_usuario_reportou) {
                    ...
                    } else if (item.tipo_registro === "achado" && item.status === "Achado" && usuarioVerificado.id === item.id_usuario_reportou) {
                    ...
                    } else if (usuarioVerificado.tipo === "ponto_coleta" && item.id_ponto_coleta === usuarioVerificado.id && (item.status === "Coletado" || item.status === "Em análise")) {
    ...
    if (formItem) {
        formItem.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                Object.keys(dadosItem).forEach(key => dadosItem[key] === undefined && delete dadosItem[key]);
                ...
                // Redirecionar para a página de detalhes do item - Corrigido
                window.location.href = `pagina_gerenciar_item.html?id=${itemSalvo.id}`; 
            } catch (error) {
